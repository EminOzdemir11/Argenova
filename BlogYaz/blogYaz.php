...
if (isset($_POST["blogekle"])) {
    ...
    $db_baslik = $_POST["baslik"];
    $db_icerik = $_POST["icerik"];

    $sql = "INSERT INTO blogs (blog_title, blog_icerik) 
    VALUES ('$db_baslik', '$db_iceik')";
    if (mysqli_query($conn, $sql)) {
        echo "Blog kaydı eklendi";
    } else {
...
<body>
    ...
    <div class="row">
        ...
        <form name="blogform" id="blogform" method="POST">
                ...
                <div class="mb-3">
                    <label for="image" class="form-label">Resim Ekle</label><br>
                    <input type="file" id="file" class="upload" accept="image/*" multiple/>
                </div>
                
                <button type="submit" name="blogekle" class="btn btn-primary">Blog Ekle</button>
            </form>
            </div>
        </div>
    </div>
